...
class PedidoController extends Controller
{
	...
	public function accessRules()
	{
		return array(			
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				'actions'=>array('index','view', 'admin','delete','update', 'detalle'),
				'users'=>array('admin'),
			),
	 ...
	 */
	public function actionUpdate($id, $idm)
	{
		$model = $this->loadModel($id);
                $model->estado = Pedido::DESPACHADO;
                $model->tiempoFinal = date('Y-m-d H:i:s');
                $model->update(array('estado', 'tiempoFinal'));
                
                if(!isset($_GET['ajax']))
			$this->redirect(array('view', 'id'=>$idm));
	}
	...
	public function actionIndex()
	{
            ...
            $cs->registerCssFile($baseUrl.'/css/mesas.css');
  
            $dataProvider = new CActiveDataProvider('Mesa', array(
                    'criteria'=>array(
                            'condition'=>'estado = ' .  Mesa::HABILITADO,
                    ),
                    'pagination'=>array(
                            'pageSize' => 100000,
                    )
